...
INDEX
Fig1.
a. (left) 394 (right)  227

Figure 1.
a. (left)  394, stats  337 (right)  227 
b.  459, stats  337
c. 2591, stats 2647
d. 2730
e. 2430
...
d. 2430
e. 2972, stats 2972
f. right 2998, stats 2998; left 3021, stats 2998
...
Figure 3.
a. 1912, stats 1847
b. left 2756, right 2851, stats 2851
...
b. 2506, 3183
c. 2765, 3154
d. left 3135, stats 3135; right 3106, stats 3106

Figure 6.
a. left 3396, right 3396
b. 3396, stats 3396
c. 3396, stats 3396
d. 3531, 3591
e. 3481, 3522
...
perform_wilcoxon_test <- function(data, trait) {
  ...
  data <- data %>%
    select(arrest_group, all_of(trait)) %>%
    drop_na()
  ...
  # Adjust p-values manually (divide by 18)
  p_adjusted <- p_value * 18
  ...
  result_df <- data.frame(
    ...
    diff_median = diff_median  # Difference in medians between groups
  )
...
# Create the plot, coloring by diff_median instead of Z_value
...
categorized_coding_stats_discovery <- categorized_coding_stats_discovery %>%
  mutate(Epilepsy_01_00001 = (Epilepsy_missense_variant_0.01 + Epilepsy_missense_variant_0.001),
...
# Filter for the top 10 lowest p-values within each source group
...
# Plot
reg_variant_plot <- ggplot(bonferroni_filtered_data, aes(y = -log10(p_value), x = pLI, color = ratio)) +
  geom_point() +
...
# Select the data points that belong to Cluster 1 (or whichever cluster you want to sub-cluster)
cluster1_data <- pca_columns[initial_clusters == 1, ]

# Perform GMM sub-clustering on Cluster 1
sub_gmm_result <- Mclust(cluster1_data, G = 2)  # Adjust G based on the number of sub-clusters you expect
...
sub_gmm_result2 <- Mclust(cluster2_data, G = 2)  # Adjust G based on the number of sub-clusters you expect

# Integrate the sub-clustering results back into your overall clustering
final_clusters[initial_clusters == 2] <- sub_gmm_result2$classification + 6  # Adjust numbering again
...
# Perform the Wilcoxon test within each ancestry group
wilcox_results <- categorized_combined_data %>%
...
diagnosis_data <- read.table("~/Desktop/diagnosis_data.txt", header = TRUE, sep = "\t", na.strings = "", stringsAsFactors = FALSE)
diagnosis_data$diagnosis <- ifelse(is.na(diagnosis_data$diagnosis), "Not_Available", diagnosis_data$diagnosis)

# Step 2: Merge and mutate the data
diagnosis_data2 <- categorized_combined_data %>%
  left_join(diagnosis_data, by = "ID") %>%
  mutate(diagnosis = if_else(is.na(diagnosis), "control", diagnosis))  # Retain 'Not_Available' from imported data


# Step 3: Calculate the percentage of instances with probability > 0.7 within each diagnosis
diagnosis_percentage <- diagnosis_data2 %>%
  group_by(diagnosis) %>%
  dplyr::summarize(
    total_count = n(),
    count_above_0.7 = sum(probability > 0.7),
    percentage_above_0.7 = (count_above_0.7 / total_count) * 100
  )

# Step 4: Extract the percentage for the control group
control_percentage <- diagnosis_percentage %>%
  filter(diagnosis == "control") %>%
  pull(percentage_above_0.7)

# Step 5: Calculate the relative difference or ratio for each diagnosis compared to the control group
diagnosis_comparison <- diagnosis_percentage %>%
  mutate(
    relative_difference = percentage_above_0.7 - control_percentage,
    ratio_to_control = percentage_above_0.7 / control_percentage
  )

# Display the result
print(diagnosis_comparison)

# Function to calculate Wilcoxon rank sum test p-values
calc_p_value <- function(score_column, diagnosis_column, data) {
  control_values <- data %>% filter(diagnosis == "control") %>% pull(!!sym(score_column))
  diagnosis_levels <- data %>% filter(diagnosis != "control") %>% pull(diagnosis) %>% unique()
  
  p_values <- sapply(diagnosis_levels, function(level) {
    diagnosis_values <- data %>% filter(diagnosis == level) %>% pull(!!sym(score_column))
    wilcox.test(diagnosis_values, control_values)$p.value
  })
  
  names(p_values) <- diagnosis_levels
  return(p_values)
}

# Calculate p-values for each score
p_value <- calc_p_value("scores", "diagnosis", diagnosis_data2)
p_values_common <- calc_p_value("common_variant_score", "diagnosis", diagnosis_data2)
p_values_epilepsy <- calc_p_value("epilepsy_variant_score", "diagnosis", diagnosis_data2)
p_values_cardiac <- calc_p_value("cardiac_variant_score", "diagnosis", diagnosis_data2)
p_values_noncoding <- calc_p_value("noncoding_variant_score", "diagnosis", diagnosis_data2)

# Combine p-values into a single data frame
p_values_df <- data.frame(
  diagnosis = names(p_values_common),
  p_value = p_value,
  common_variant_p_value = p_values_common,
  epilepsy_variant_p_value = p_values_epilepsy,
  cardiac_variant_p_value = p_values_cardiac,
  noncoding_variant_p_value = p_values_noncoding
)

# Calculate the control median
control_medians <- diagnosis_data2 %>%
  filter(diagnosis == "control") %>%
  dplyr::summarize(
    control_scores = median(scores, na.rm = TRUE),
    control_common_variant_score = median(common_variant_score, na.rm = TRUE),
    control_epilepsy_variant_score = median(epilepsy_variant_score, na.rm = TRUE),
    control_cardiac_variant_score = median(cardiac_variant_score, na.rm = TRUE),
    control_noncoding_variant_score = median(noncoding_variant_score, na.rm = TRUE)
  )

# Prepare data for bubble plot using median differences
bubble_data <- diagnosis_data2 %>%
  group_by(diagnosis) %>%
  dplyr::summarize(
    scores = median(scores, na.rm = TRUE),
    common_variant_score = median(common_variant_score, na.rm = TRUE),
    epilepsy_variant_score = median(epilepsy_variant_score, na.rm = TRUE),
    cardiac_variant_score = median(cardiac_variant_score, na.rm = TRUE),
    noncoding_variant_score = median(noncoding_variant_score, na.rm = TRUE)
  ) %>%
  mutate(
    diff_scores = scores - control_medians$control_scores,
    diff_common_variant_score = common_variant_score - control_medians$control_common_variant_score,
    diff_epilepsy_variant_score = epilepsy_variant_score - control_medians$control_epilepsy_variant_score,
    diff_cardiac_variant_score = cardiac_variant_score - control_medians$control_cardiac_variant_score,
    diff_noncoding_variant_score = noncoding_variant_score - control_medians$control_noncoding_variant_score
  ) %>%
  left_join(p_values_df, by = "diagnosis")


# Melt the data for plotting
bubble_data_melted <- bubble_data %>%
  pivot_longer(cols = c(diff_scores,diff_common_variant_score, diff_epilepsy_variant_score, diff_cardiac_variant_score, diff_noncoding_variant_score),
               names_to = "variable", values_to = "value")%>%
  pivot_longer(cols = c(p_value, common_variant_p_value, epilepsy_variant_p_value, cardiac_variant_p_value, noncoding_variant_p_value),
               names_to = "p_value_type", values_to = "p_value")

# Filter the melted data to align p-values with their corresponding scores
bubble_data_filtered <- bubble_data_melted %>%
  filter((variable == "diff_scores" & p_value_type == "p_value") |
         (variable == "diff_common_variant_score" & p_value_type == "common_variant_p_value") |
         (variable == "diff_epilepsy_variant_score" & p_value_type == "epilepsy_variant_p_value") |
         (variable == "diff_cardiac_variant_score" & p_value_type == "cardiac_variant_p_value") |
         (variable == "diff_noncoding_variant_score" & p_value_type == "noncoding_variant_p_value")) %>%
  filter(diagnosis != "NA")%>%
  filter(diagnosis != "Other") %>%
  filter(diagnosis != "control") 

# Create bubble plot with p-value labels
ggplot(bubble_data_filtered, aes(x = diagnosis, y = variable, size = value, color = -log10(p_value))) +
  geom_point(alpha = 0.6) +
  geom_text(aes(label = round(p_value, 2)), color = "black") + # Add p-value labels
  scale_size_continuous(range = c(3, 10)) +  # Adjust size range as needed
  scale_color_gradientn(colors = c("orange", "red"), breaks = c(1.301, 5,10,20), space = "Lab") + # Custom color scale
  theme_minimal() +
  labs(title = "Bubble Plot of Scores by Diagnosis",
       x = "Diagnosis",
       y = "Score Type",
       size = "Scaled Value",
       color = "-log10(P-value)") +
...
ggplot(bubble_data_filtered, aes(x = diagnosis, y = variable, size = value, color = -log10(p_value))) +
  geom_point(alpha = 0.6) +
  #geom_text(aes(label = round(p_value, 2)), color = "black") + # Add p-value labels
  scale_size_continuous(range = c(0, 10)) +  # Adjust size range as needed
  scale_color_gradientn(colors = c("orange", "red"), breaks = c(1.301, 5,10,20), space = "Lab") + # Custom color scale
  theme_minimal() +
  labs(title = "Bubble Plot of Scores by Diagnosis",
       x = "Diagnosis",
